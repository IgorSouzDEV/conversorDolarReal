document.addEventListener('DOMContentLoaded', function () {
    ...
    function finalValue() {
        ...
        if (resultDiv) {
            resultDiv.textContent = 'Valor convertido em Real (BRL): R$ ' + realValue.toFixed(2);
        } else {
            // Se não existir, criar uma nova div dinamicamente com o valor convertido
            resultDiv = document.createElement('div');
            resultDiv.textContent = 'Valor convertido em Real (BRL): R$ ' + realValue.toFixed(2);
            resultDiv.id = 'resultDiv';
            resultDiv.className = 'd-flex justify-content-center mt-5 alert alert-success w-50 mx-auto'; // Adicione classes Bootstrap para o estilo
